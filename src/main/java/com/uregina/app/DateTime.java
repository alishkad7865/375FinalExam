...
public class DateTime {
	 ...
	 * @return the value of (d1-d2) in minutes it may be a negative value Throws
	 *         MoreThanOneDayException if d1,d2 are not at the same date or not a
	 *         consequative days. usefeul functions that you may use (class: Date ,
	 *         method: equal) (class: Time12 , method: subtract) (class: Date ,
	 *         method: nextDate)
	 */
	public static int subtract(DateTime d1, DateTime d2) throws MoreThanOneDayException {
		int diff = 0;
		// Todo: add your code here
		if (Date.equal(d1.date, d2.date) || d1.getDate().nextDate() == d2.getDate()
				|| d2.getDate().nextDate() == d1.getDate()) {
			diff = Time12.subtract(d1.time, d2.time);
			if (d1.getDate().nextDate() == d2.getDate()) {
				diff += 1440;
			}
			else if(d2.getDate().nextDate() == d1.getDate()){
				diff -= 1440;
			}
